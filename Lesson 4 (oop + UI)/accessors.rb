    ...
    def attr_accessor_with_history(*names)
      names.each do |name|
        ...
        define_method("#{name}=") do |value|
          ...
          history_values = instance_variable_get(name_history_variable) || []
          instance_variable_set(name_history_variable, history_values << value)
        end
      end
    end
    ...
    def strong_attr_accessor(name, type)
      ...
      define_method("#{name}=".to_sym) do |value|
        unless value.is_a?(type)
          raise TypeError, "Тип не соответствует типу #{type}"
        end
